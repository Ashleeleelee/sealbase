                  ...
                  onClick={(() => {
                    ...
                    return async () => {
                      ...
                      if (count >= 5) {
                        ...
                        if (!pwd) return;
                        // SHA-256 纯 JS 实现，兼容 HTTP 环境（crypto.subtle 仅在 HTTPS 可用）
                        const sha256 = async (str) => {
                          ...
                          const blen=BigInt(len*8);
                          for(let i=7n;i>=0n;i--)bytes.push(Number((blen>>(i*8n))&0xffn));
                          for(let i=0;i<bytes.length;i+=64){
                            const w=[];
                            for(let j=0;j<16;j++)w.push((bytes[i+j*4]<<24)|(bytes[i+j*4+1]<<16)|(bytes[i+j*4+2]<<8)|bytes[i+j*4+3]);
                            for(let j=16;j<64;j++){const s0=((w[j-15]>>>7)|(w[j-15]<<25))^((w[j-15]>>>18)|(w[j-15]<<14))^(w[j-15]>>>3);const s1=((w[j-2]>>>17)|(w[j-2]<<15))^((w[j-2]>>>19)|(w[j-2]<<13))^(w[j-2]>>>10);w.push((w[j-16]+s0+w[j-7]+s1)>>>0);}
